...
class GDMLBox(GDMLcommon) :
   ...
   def createGeometry(self,fp):
       ...
       if all((fp.x,fp.y,fp.z)) :
          ...
          fp.Shape = translate(Part.makeBox(x,y,z),
	                       FreeCAD.Vector(-x/2,-y/2,-z/2))
...
class GDMLCone(GDMLcommon) :
   ...
   def createGeometry(self,fp):
       if (hasattr(fp,'rmin1') and hasattr(fp,'rmax1') and \
           ...
           hasattr(fp,'z')) :
          mul = GDMLShared.getMult(fp)
          rmin1 = max(mul * fp.rmin1,0.0)
          rmin2 = max(mul * fp.rmin2,0.0)
          rmax1 = max(mul * fp.rmax1,rmin1)
          rmax2 = max(mul * fp.rmax2,rmin2)
          if(rmax1<=0.0 and rmax2<=0.0): return
          if(fp.z<=0.0): return
          z = mul * fp.z
          ...
          if rmax1 != rmax2 :
             cone1 = Part.makeCone(rmax1,rmax2,z, FreeCAD.Vector(0,0,0), FreeCAD.Vector(0,0,1),  angleDeltaPhiDeg)
          else :  
             cone1 = Part.makeCylinder(rmax1,z, FreeCAD.Vector(0,0,0), FreeCAD.Vector(0,0,1),  angleDeltaPhiDeg)
          ...
          if (rmin1 != 0 or rmin2 != 0 ) :
             if rmin1 != rmin2 :
                cone3 = cone1.cut(Part.makeCone(rmin1,rmin2,z))
             else :
                cone3 = cone1.cut(Part.makeCylinder(rmin1,z))
          else :
             cone3 = cone1
          
          fp.Shape = translate(cone3,FreeCAD.Vector(0,0,-z/2))
...
class GDMLPolyhedra(GDMLcommon) :
   ...
   def createGeometry(self,fp):
       ...
       # faceList initialisation in order (instead of append function)
       faceList = [0 for i in range((len(parms)-1)*numsides*6)]
       indiceFaceList=0
           ...
           for indiceParm in range(len(parms)-1):
               ...
               if ( deltaphi_rad < 2*math.pi ):
                   ...
                   if indiceSide == numsides-1 :
                       faceList[indiceFaceList] = (Part.Face(Part.makePolygon([rmin_zmin_phimax,
                                                  rmax_zmin_phimax,
                                                  rmaxp1_zmax_phimax,
                                                  rminp1_zmax_phimax,
                                                  rmin_zmin_phimax])))
       ...
       fp.Shape = Part.makeShell(faceList[:indiceFaceList])
       del faceList[:]
       del faceList
...
class GDMLSphere(GDMLcommon) :
   ...
   def createGeometry(self,fp):
       ...
       deltathetaRad = max(0.0, min(getAngleRad(fp.aunit, fp.deltatheta),math.pi))
       startthetaRad = max(0.0, min(getAngleRad(fp.aunit, fp.starttheta),math.pi))
...
class GDMLTube(GDMLcommon) :
   ...
   def createGeometry(self,fp):
       #for bad definition
       if fp.rmax<=0.0 : return  
       if fp.rmax<=fp.rmin : return
	   ...
	   # startphi
           if fp.startphi != 0 :
                cyl1.rotate(FreeCAD.Vector(0,0,0),
                            FreeCAD.Vector(0,0,1), 
                            getAngleDeg(fp.aunit,fp.startphi))

       else:  # no deltaphi definition: simple cylinder
           cyl1 = Part.makeCylinder(rmax, z)   

       if rmin > 0 :
           # one "inside" cylinder cuts "outside cylinder
